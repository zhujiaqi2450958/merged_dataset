...
# ========== 自动获取项目根目录（关键修改，不用再写死路径） ==========
# 这行代码会自动根据当前脚本位置，找到项目根目录，换电脑也能用
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
IMAGE_DIR = os.path.join(BASE_DIR, "images")  # 图片文件夹
CSV_PATH = os.path.join(BASE_DIR, "merged_annotations.csv")   # CSV文件
...
if not os.path.exists(IMAGE_DIR):
    ...
    print("请确认 'images' 文件夹已复制到项目根目录")
    exit()
if not os.path.exists(CSV_PATH):
    print(f"❌ CSV文件不存在: {CSV_PATH}")
    print("请确认 'merged_annotations.csv' 文件已复制到项目根目录")
    exit()
print("✅ 路径检查通过")
...
if len(valid_images) == 0:
    print("\n❌ 没有找到匹配的图片！")
    print("CSV中的文件名示例:", csv_image_names[:5])
    print("文件夹中的文件名示例:", actual_images[:5])
    exit()
...
print(f"\n每张图片的标注框数量(前10张):")
for i, (img_name, boxes) in enumerate(list(image_to_boxes.items())[:10]):
    print(f"  {img_name}: {len(boxes)} 个框")
...
report_path = os.path.join(BASE_DIR, 'preprocess_report.txt')
...
print("="*60)
print(f"\n交付物:")
print(f"  1. {OUTPUT_DIR}/ - 预处理后的数据集")
print(f"  2. {config_path} - YOLO配置文件")
print(f"  3. {report_path} - 预处理报告")